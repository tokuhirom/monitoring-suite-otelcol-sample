...
WORDS = [
    # アプリケーション関連
    "application", "server", "client", "database", "connection", "request", "response",
    "service", "api", "endpoint", "handler", "middleware", "controller", "model",
    
    # 動作関連
    "starting", "stopping", "processing", "connecting", "disconnecting", "loading",
    "saving", "updating", "creating", "deleting", "sending", "receiving", "parsing",
    
    # 状態関連
    "success", "failed", "error", "warning", "info", "debug", "completed", "timeout",
    "ready", "busy", "idle", "active", "inactive", "pending", "cancelled",
    
    # 技術用語
    "http", "https", "tcp", "ssl", "tls", "json", "xml", "yaml", "config",
    "cache", "session", "token", "auth", "login", "logout", "user", "admin",
    
    # 数値・識別子
    "id", "uuid", "hash", "key", "value", "count", "size", "length", "port",
    "version", "build", "release", "commit", "branch", "tag",
    
    # エラー関連
    "exception", "stack", "trace", "panic", "crash", "abort", "retry", "fallback",
    "circuit", "breaker", "health", "check", "monitor", "alert", "notification"
]
...
def generate_random_message():
    ...
    replacements = {
        ...
        "operation": random.choice(["database_query", "api_call", "file_write", "cache_update"]),
        "space": random.randint(1, 10),
        "days": random.randint(1, 30),
        "size": random.randint(100, 1000),
        "error": random.choice(["connection_refused", "timeout", "invalid_credentials", "permission_denied"]),
        "timeout": random.randint(5, 30),
        "resource": f"/{random.choice(['admin', 'api', 'data'])}/{random.choice(['users', 'files', 'config'])}",
        "service": random.choice(["database", "cache", "auth", "notification", "payment"]),
