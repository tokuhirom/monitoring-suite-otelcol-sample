...
def test_cpu_metrics_with_labels():
    """CPU メトリクスが取得できて、適切なラベルが付与されていることを確認"""
    ...
    response = requests.get(
        ...
        params={"query": 'system_cpu_time_seconds_total{region="tokyo",sample_name="my-service"}'},
        timeout=10
    )
    ...
    # メトリクスが取得できていることを確認
    assert len(results) > 0, "No CPU metrics found with required labels"
    ...
    # ラベルの確認
    assert labels.get("__name__") == "system_cpu_time_seconds_total", "Metric name mismatch"
    assert labels.get("region") == "tokyo", "External label 'region=tokyo' not found"
    assert labels.get("sample_name") == "my-service", "Resource attribute 'sample_name=my-service' not found"

    # 値の確認
    value = float(value_data[1])
    assert value >= 0, f"Invalid metric value: {value}"

    print(f"✓ CPU metrics found: {len(results)} time series")
    print(f"✓ Sample metric: {labels.get('__name__')}")
    print(f"✓ Labels: region={labels.get('region')}, sample_name={labels.get('sample_name')}")
    print(f"✓ Value: {value}")
